...
\* VARIABLES
\* -------------------------------------------------------------------------
VARIABLES links, monitors, config, terminated, cache, deps

vars == <<links, monitors, config, terminated, cache, deps>>
...
TypeOK ==
  ...
  /\ deps      \in [Pids -> SUBSET (Pids \X Apps \X Keys)]
...
Terminate(pid) ==
  ...
  /\ monitors' = [p \in Pids |->
       IF p = pid THEN {}
       ELSE monitors[p] \ {pid}]
  /\ cache' = cache \ deps[pid]   \* invalidate only entries that depended on this pid
  /\ deps' = [p \in Pids |->
       IF p = pid THEN {}
       ELSE deps[p] \ deps[pid]]   \* also clean up stale dep references

FetchAction(pid, app, key) ==
  \* Only live pids can fetch
  /\ pid \notin terminated
  \* Determine the blocking ancestor BEFORE potentially adding to cache
  /\ LET ancestor == BFS(<<<<pid, SrcLinks>>>>, {})
         result  == FetchCached(pid, app, key)
     IN  /\ IF result = FetchErr
            THEN /\ cache' = cache \cup {<<pid, app, key>>}
                 \* Record dependency: which config pid blocked this miss
                 /\ deps' = IF ancestor /= WalkNone
                     THEN [p \in Pids |->
                          IF p = ancestor
                          THEN deps[p] \cup {<<pid, app, key>>}
                          ELSE deps[p]]
                     ELSE deps
            ELSE UNCHANGED <<cache, deps>>
